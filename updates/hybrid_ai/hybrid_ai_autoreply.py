...
def _update_notification_text(manifest: dict[str, Any]) -> str:
    ...
    body += (
        "\n\n💾 Настройки, шаблоны и сохранённая конфигурация остаются на месте."
        "\n♻️ После установки перезапустите Cardinal, чтобы загрузился новый код."
        "\n🛡 Обновление скачивается только по HTTPS и проверяется по SHA-256, UUID, VERSION и синтаксису Python."
    )
    return body
